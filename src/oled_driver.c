...
void oled_init(void) {
    ...
    RST = 0; __delay_ms(10); RST = 1; __delay_ms(10);
    oled_cmd(0xAD); oled_cmd(0x8B);     // Charge pump ON
    oled_cmd(0x33);                // VPP = 9V
    oled_cmd(0xA1);                // Flip horizontal
    oled_cmd(0xC8);                // Flip vertical
    oled_cmd(0x81); oled_cmd(0xFF);     // Max contrast
...
void oled_clear(void) {
    for(uint8_t p = 0; p < 8; p++) {
        oled_set_pos(p, 0);
        for(uint8_t c = 0; c < 128; c++) oled_send_data(0x00);
    }
}
